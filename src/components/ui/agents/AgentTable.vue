<template>
  <div class="mt-8 flow-root">
    <div class="-mx-4 -my-2 sm:-mx-6 lg:-mx-8">
      <div
        class="inline-block min-w-full py-2 align-middle table-height overflow-y-scroll border-b-2"
      >
        <table class="min-w-full border-separate border-spacing-0">
          ...
          <tbody v-if="agents">
            <tr
              v-for="(person, personIdx) in agents"
              :key="person.id"
              class="hover:bg-indigo-50"
            >
              <td
                :class="[
                  personIdx !== agents.length - 1
              ...
              >
                <div
                  class="w-8 h-8 flex flex-row items-center justify-center text-white rounded-full"
                  :style="{ backgroundColor: person.fields.color }"
                >
                  {{ person.fields.agent_name.charAt(0)
                  }}{{ person.fields.agent_surname.charAt(0) }}
                </div>
              </td>
              <td
                :class="[
                  personIdx !== agents.length - 1
                    ? 'border-b border-gray-200'
                    : '',
                  'whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-2 lg:pl-3',
                ]"
              >
                {{ person.fields.agent_name }} {{ person.fields.agent_surname }}
              </td>
              <td
                :class="[
                  personIdx !== agents.length - 1
              ...
              >
                {{
                  person.fields.appointments
                    ? person.fields.appointments.length
                    : 0
                }}
              </td>
              <td
              ...
              >
                <div class="flex flex-row gap-x-4">
                  <button
                    disabled
                    class="cursor-not-allowed flex justify-center items-center w-7 h-7 bg-gray-200 rounded-md text-black hover:bg-indigo-500 hover:text-white duration-100"
                  >
