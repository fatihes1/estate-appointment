...
const handleOnClickAppointment = (appointment) => {
  contactData.value = {
    ...
    is_cancelled: appointment.fields.is_cancelled
...
<template>
  <div
    class="w-full px-4 border-2 border-gray-300 flex flex-col md:flex-row justify-around py-2 rounded-lg mt-3 cursor-pointer hover:border-indigo-500 hover:bg-indigo-50 transition duration-200 ease-in-out"
    :class="index % 2 === 0 ? 'bg-gray-200' : 'bg-gray-50'"
    @click="clickable ? handleOnClickAppointment(appointment) : null"
  >
    <div
      v-if="clickable"
      class="w-full px-4 flex flex-col md:flex-row justify-around"
    >
      ...
      <div
        class="w-full md:w-4/12 flex justify-center items-center mb-4 md:mb-0"
      >
        <appointment-status
    ...
    <div
      v-else
      class="flex flex-col justify-center items-center gap-y-1 w-full"
    >
      <div>
        <address-card :location="appointment.fields.appointment_address" />
      </div>
