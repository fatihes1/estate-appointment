...
import {EllipsisHorizontalIcon, CalendarIcon, MapPinIcon} from "@heroicons/vue/20/solid";
...
<template>
  <li v-if="appointment" :key="appointment.id" class="relative flex space-x-6 py-6 xl:static">
          ...
          <MenuItems class="absolute right-0 z-10 mt-2 w-36 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div class="py-1">
              <MenuItem v-slot="{ active }">
                ...
                <MenuItem v-slot="{ active }">
                  <div @click="handleOnOkCancelAppointment(appointment.id)"  :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-sm cursor-pointer']">Cancel</div>
                </MenuItem>
            </div>
          </MenuItems>
      </transition>
