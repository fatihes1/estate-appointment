    ...
    <div class="-mx-4 -my-2 sm:-mx-6 lg:-mx-8">
      ...
      >
        <table class="min-w-full border-separate border-spacing-0">
          ...
          <tbody>
            <tr
              v-for="(person, personIdx) in contacts"
              :key="person.id"
              class="hover:bg-indigo-50"
            >
              <td
                :class="[
                  personIdx !== contacts.length - 1
                    ? 'border-b border-gray-200'
                    : '',
                  'whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 lg:pl-8',
                ]"
              >
                {{ person.fields.contact_name }}
                {{ person.fields.contact_surname }}
              </td>
              <td
                :class="[
                  personIdx !== contacts.length - 1
              ...
              >
                <div class="flex flex-row gap-x-4">
                  <button
                    disabled
                    @click="openEditModal(person.id)"
                    class="flex justify-center items-center w-7 h-7 bg-gray-200 rounded-md text-black hover:bg-indigo-500 hover:text-white duration-100 cursor-not-allowed"
                  >
